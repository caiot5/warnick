...
# 2022-11-12    2.1.2    Fixed a bug that allowed the main homepage of a domain
#                        to be downloaded twice by error. Added additional
#                        handling for IA HTTP status codes to account for
#                        connection throttling.
# 2022-11-12    2.1.3    Kept the format of the log file header the same when
#                        using debug mode, assisting usage with the development
#                        portal on Protoweb. It does not affect functionality
#                        when Warnick is used in console.
# 2022-11-15    2.1.4    Removed 2.1.2, it was causing issues. Instead, added
#                        more robust checking of downloaded files so that they
#                        do not end up overwriting files that were downloaded
#                        earlier. Although the link list can usually mitigate
#                        downloading the same files over and over again, it
#                        will not eliminate the problem, for example, the same
#                        file can be referred to as /dir/index.html and /dir/.
#                        Combined "skipping link to other host" log entries
#                        into a single-line log entry to reduce log spamming.
# -----------------------------------------------------------------------------
#
...
#
# We recommend using 3 or higher.
cooldown=15
...
function scanlinks {
    ...
    if [[ ! -z $linkoffset ]]; then
      ...
      if [[ -z "$directlink" ]]; then
        ...
        if [[ ! "${linkhost,,}" == "$host" ]]; then
          ...
          if [[ -z "$otherhostlinks" ]]; then
            otherhostlinks="$directlink"
          else
            # add to list if it does not already exist:
            if [[ ! $otherhostlinks == *"$directlink"* ]]; then
              otherhostlinks="${otherhostlinks}, $directlink"
            fi
          fi
          skiplink=1
        fi
  ...
  if [[ ! -z "$otherhostlinks" ]]; then
    log 1 "Notice: Skipping links leading to other hosts: $otherhostlinks"
  fi
}
...
if [[ "$owner" != "$defaultowner" ]]; then
  ...
  if [[ ! -z "$host" ]]; then
    # create default .htaccess file with site date
    echo "ArchiveDate $datestring" >> ./sites/$host/.htaccess
